//! ReactOS WOW64 - Capa de compatibilidad para aplicaciones 32-bit
//! 
//! Implementa la capa WOW64 que permite ejecutar aplicaciones 32-bit
//! en un sistema operativo 64-bit nativo.
...
impl Wow64 {
    ...
    unsafe fn switch_to_32bit(&self) {
        ...
        core::arch::asm!(
            ...
            "push {1:e}",
            "retf",
            in(reg) 0x23u32, // Selector de código 32-bit
            in(reg) self.context_32.eip,
        );
    }
...
impl Thunk {
    ...
    fn generate_thunk_code(addr_64: u64) -> Result<[u8; 32]> {
        let mut code = [0u8; 32];
        
        // Código del thunk en ensamblador
        // 1. Guardar registros 32-bit
        code[0] = 0x50; // push eax
        code[1] = 0x51; // push ecx
        code[2] = 0x52; // push edx
        code[3] = 0x53; // push ebx
        
        // 2. Llamar a la función 64-bit
        code[4] = 0x48; // REX.W
        code[5] = 0xB8; // mov rax, imm64
        code[6..14].copy_from_slice(&addr_64.to_le_bytes());
        code[14] = 0xFF; // call rax
        code[15] = 0xD0;
        
        // 3. Restaurar registros 32-bit
        code[16] = 0x5B; // pop ebx
        code[17] = 0x5A; // pop edx
        code[18] = 0x59; // pop ecx
        code[19] = 0x58; // pop eax
...
/// Panic handler para WOW64
#[panic_handler]
fn panic(_info: &PanicInfo) -> ! {
    // Log del panic
    // En un sistema real aquí se mostraría el error
    
    // Terminar proceso 32-bit
    unsafe {
        core::arch::asm!("int3");
    }
